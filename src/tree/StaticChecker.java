...
import tree.ExpNode.RecordNode;
...
public class StaticChecker implements DeclVisitor, StatementVisitor, 
    ...
    public ExpNode visitRecordNode(ExpNode.RecordNode node) {
        ...
        List<Field> fields = node.getType().getFieldList();
        
        //Transforms field list
        for(ExpNode x: node.getList()) {
            
            ExpNode newNode = x.transform(this);
            
            if(newNode.getType() instanceof Type.ReferenceType) {
                //Dereferences any new reference nodes
                Type newType = newNode.getType().optDereferenceType();
                ExpNode noder= newType.coerceExp(newNode);
                newNode = noder;
            }
        ...
        if(list.size() != fields.size()) {
            staticError("Incorrent number of expressions in record constructor",node.getLocation());
        } else {
            for(int i=0; i < list.size(); i++) {
                
                
                if(!(node.getList().get(i).getType().equals(fields.get(i).getType()))) {
                    staticError("Expression type does not match record constructor", node.getList().get(i).getLocation());
                } else {
                    node.addField(fields.get(i).getId(), node.getList().get(i));
                }
            }
        }
    ...
    public ExpNode visitFieldNode(ExpNode.FieldNode node) {
        ...
        //Parent handling, repeats until parent is of record type cause recursion 
        //(everything before record will be another field node)
        ExpNode tempnode = node.getParent().transform(this);
        Type basetype = tempnode.getType().optDereferenceType();
        tempnode.setType(tempnode.getType().optDereferenceType());
        
        ExpNode newParent = basetype.coerceExp(tempnode);
        node.setParent(newParent); 
        
        //Checking for existence of ID
        if (node.getParent().getType() instanceof Type.RecordType) {
            if(!((Type.RecordType)node.getParent().getType()).containsField(node.getId())) {
                staticError("Record does not contain field " + node.getId(), node.getLocation());
            }
        } else if ((!(node.getParent() instanceof ExpNode.FieldNode)) && 
                (node.getParent() instanceof ExpNode.VariableNode)) {
            //If not a record, check if is not another field node, if not, error
            //If not a variable, allows static checker to throw variable error instead
            staticError("Invalid record", node.getLocation());
        }
    ...
    public ExpNode visitNewPointerNode(ExpNode.NewPointerNode node) {
        ...
        if(!(node.getType() instanceof Type.PointerType)) {
            Type.PointerType newType = new Type.PointerType(node.getType());
    ...
    //Should be done
    public ExpNode visitPointerDerefNode(ExpNode.PointerDerefNode node) {
        beginCheck("PointerDeref");
        
        ExpNode value = node.getValue().transform(this);
        Type baseType = value.getType().optDereferenceType();
        
        if((!(baseType instanceof Type.PointerType)) && (!(value instanceof ExpNode.PointerDerefNode))) {
            staticError("type must be pointer", node.getLocation());
        } else {
            value = baseType.coerceExp(value);
